  ...
  if (items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link href="/products">
          <Button className="gap-2">Browse Products <ArrowRight className="w-4 h-4" /></Button>
        </Link>
      </div>
    );
  }
            ...
            <Card key={item.id} className="overflow-hidden">
              <CardContent className="p-4 sm:p-6">
                <div className="flex gap-4">
                  <Link href={`/products/${item.products?.slug}`} className="w-20 h-20 sm:w-24 sm:h-24 shrink-0 relative overflow-hidden rounded-lg bg-secondary">
                    {item.products?.image_url ? (
                      <Image src={item.products.image_url} alt={item.products.name} fill className="object-cover" />
                    ) : (
                      <div className="flex items-center justify-center h-full">
                        <span className="text-2xl font-bold text-primary/20">{item.products?.name?.charAt(0)}</span>
                      </div>
                    )}
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link href={`/products/${item.products?.slug}`}>
                      <h3 className="font-semibold hover:text-primary transition-colors line-clamp-1">{item.products?.name}</h3>
                    </Link>
                    <p className="text-sm text-muted-foreground mt-0.5">{item.products?.categories?.name}</p>
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-primary">${(item.products?.price * item.quantity).toFixed(2)}</span>
                        <button
