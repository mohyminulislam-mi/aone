      ...
      <div className="bg-primary text-primary-foreground">
        <div className="container mx-auto px-4 py-1.5 text-xs flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-4">
            ...
            {profile?.is_admin && (
              <Link href="/admin" className="hover:underline font-medium">Admin Dashboard</Link>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1 ml-8">
            <Link href="/" className="px-3 py-2 text-sm font-medium text-foreground/80 hover:text-primary transition-colors rounded-md hover:bg-secondary">
              Home
            </Link>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-52">
                <DropdownMenuItem asChild>
                  <Link href="/products" className="font-medium">All Products</Link>
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                {categories.map((cat) => (
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-border bg-background animate-fade-in">
          <div className="container mx-auto px-4 py-4 space-y-2">
            ...
            {profile?.is_admin && (
              <Link href="/admin" onClick={() => setMobileOpen(false)} className="block px-3 py-2 text-sm font-medium hover:bg-secondary rounded-lg text-primary">
                Admin Dashboard
              </Link>
            )}
          </div>
